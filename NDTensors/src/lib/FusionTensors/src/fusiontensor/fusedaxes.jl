...
GradedAxes.blocklabels(fa::FusedAxes) = GradedAxes.blocklabels(fused_axis(fa))
...
function compute_inner_ranges(outer_legs::Tuple{Vararg{AbstractGradedUnitRange}})
  ...
  outer_blocklabels = GradedAxes.blocklabels.(outer_legs)
  ...
  for (i_block, it) in
    ...
    block_axis = GradedAxes.fusion_product(getindex.(outer_blocklabels, Tuple(it))...)
    block_sectors = GradedAxes.blocklabels(block_axis)
    for (i1, i2) in find_shared_indices(allowed_sectors, block_sectors)
      m[i_block + 1, i1] =
        prod(getindex.(outer_blocklengths, it)) * length(block_axis[BlockArrays.Block(i2)])
    end
  end
  mcs = cumsum(m; dims=1)
...
function Base.intersect(left::FusedAxes, right::FusedAxes)
  left_labels = GradedAxes.blocklabels(left)
  right_labels = GradedAxes.blocklabels(right)
...
function allowed_outer_blocks_sectors(
  ...
  @assert left_labels[left_indices] == GradedAxes.blocklabels(right)[right_indices]
